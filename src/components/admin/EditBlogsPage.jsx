...
import { FiSave, FiX, FiRefreshCw, FiTrash2, FiChevronDown, FiLock, FiArrowLeft, FiSliders, FiCalendar, FiCheckCircle, FiCopy, FiImage, FiUploadCloud, FiEdit3 } from 'react-icons/fi';
import { getBlogByIdService, updateBlogService, uploadInlineImageService } from '../service/blog.service';
...
const EditBlogsPage = () => {
    ...
    const fetchBlog = useCallback(async () => {
        ...
        try {
            ...
            const blog = extractBlog(response);
            if (!blog) {
                triggerToast('Blog data could not be found.');
                return;
            }
    ...
    const handleCopySlug = async () => {
        try {
            ...
            triggerToast('Slug copied successfully!');
        } catch (error) {
            console.error('Copy slug failed:', error);
            triggerToast('Unable to copy slug.');
        }
    };
    ...
    const handleFileChange = (e) => {
        ...
        if (
            ...
            triggerToast('Cover image must be less than 10MB.');
            e.target.value = '';
            return;
        }
        if (
        ...
        triggerToast('Featured image selected.');
    };
    ...
    const removeCoverImage = () => {
        ...
        triggerToast('Featured image removed.');
    };
    ...
    const handleInlineImageChange =
        async (e) => {
            ...
            if (
                ...
                triggerToast('Inline image must be less than 10MB.');
                e.target.value = '';
                return;
            }
            try {
                ...
                triggerToast('Inline image uploaded successfully!');
            } catch (error) {
    ...
    const buildFormData = (targetStatus) => {
        ...
        if (formattedPublicationDate) {
            ...
            formData.append('publishDate', formattedPublicationDate);
        }
    ...
    const handleUpdateBlog = async (targetStatus, successMessage) => {
        if (!blogId) {
            triggerToast('Invalid blog ID.');
            return;
        }
        ...
        try {
            ...
            // console.log('Update Status:',targetStatus);
            const response =
                await updateBlogService(blogId, formData);
