...
import { FiSearch, FiTrash2, FiX, FiInfo, FiUpload, FiLock, FiCheckCircle, FiClock, } from 'react-icons/fi';
import { IoTimerOutline } from "react-icons/io5";
import { getTrashBlogsService, restoreBlogService, permanentlyDeleteBlogService, } from '../service/blog.service';
...
export default function TrashPage() {
  ...
  const fetchTrashBlogs = async () => {
    try {
      ...
      console.error('Fetch Trash Blogs Error:', error);
  ...
  const handleRestore = async (id) => {
    try {
      ...
      setErrorMessage('');
      // console.log('Restore Blog ID:',id);
      const response = await restoreBlogService(id);
      // console.log('Restore Blog Response:',response);
      setPosts((prev) =>
        prev.filter(
          (post) =>
            post.id !== id
  ...
  const handleDelete = async (id) => {
    try {
      setActionLoading(true);
      setErrorMessage('');
      // console.log('Permanent Delete Blog ID:',id);
      const response =
